const ja_JP = {
    info: {
        title: "ECHO 按钮",
        info: "通知",
        null: "空",
        desktopVersion: "ダウンロード: <a href=\"https://drive.google.com/open?id=1Fx6vwqoML1gyWD3XYCVD8IX_bb427uK5\" target=\"_blank\">https://drive.google.com/open?id=1Fx6vwqoML1gyWD3XYCVD8IX_bb427uK5</a>",
        desktopInfo: "デスクトップ版をダウンロード",
        stafflist: "<center>めあめあボタン製作委員会</center><br><br>オーディオクリップ: 寂寞时空, plasc, grayimp, Claymorist, 溯时的莫名<br>i18n: 现充酱大危机！, stan<br>デスクトップ版制作: 夕舞八弦<br>Web制作: Zyzsdy",
        toGithub: "Githubで翻訳に参加、音声を追加および提案をしてください。",
        notOfficial: "このサイトはファン作品であり、公式とは関係ありません。"
    },
    action: {
        toggleNavbar: "ナビゲーションバーを切り替える",
        close: "閉じる",
        copy: "コピー",
        control: "コントロール",
        stopvoice: "現在の声を止める"
    },
    lang: {
        'zh-CN': "简体中文",
        'en-US': "English",
        'ja-JP': "日本語"
    }
}
export default ja_JP;